...
SDL_Surface *power_map_new(SDL_Surface *surface, SDL_Surface *edge_map,
  ...
  for (int j = image_h - 1; j >= 0; j--) {
    for (int i = 0; i < image_w; i++) {
      replace_pixel =
          (int)(gsl_matrix_get(power_map_matrix, i, j) * divisor_factor);
      set_pixel(power_map, replace_pixel, replace_pixel, replace_pixel, sa, i,
                j);
    }
  }
      ...
      if (j != image_h - 1) {
        if (current_index == 1) {
          mid = gsl_matrix_get(power_map_matrix, (size_t)(current_index),
                               (size_t)(j + 1));
          right = gsl_matrix_get(power_map_matrix, (size_t)(current_index + 1),
                                 (size_t)(j + 1));
    ...
    for (int j = 0; j < image_h; j++) {
      ...
      for (int i = 0; i < image_w; i++) {
        if (to_remove[j] != i) {
          ...
          gsl_matrix_set(power_map_update_matrix, index, j,
                         gsl_matrix_get(power_map_matrix, i, j));
