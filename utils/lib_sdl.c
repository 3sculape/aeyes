...
void create_window(size_t width, size_t height, SDL_Window **window,
                   SDL_Renderer **renderer) {
  SDL_CreateWindowAndRenderer(width, height, SDL_WINDOW_HIDDEN, window,
                              renderer);
}

SDL_Texture *create_texture(SDL_Renderer *renderer, size_t width,
                            size_t height) {
  SDL_Texture *texture;
...
void savePNG(char *filename, SDL_Surface *surface) {
  ...
  png_set_IHDR(png_ptr, info_ptr, surface->w, surface->h, depth,
...
void saveJPG(char *filename, SDL_Surface *surface) {
  ...
  unsigned char *raw_image =
      (unsigned char *)malloc(surface->w * surface->h * cinfo.num_components);
...
SDL_Surface *texture_to_surface(SDL_Texture *texture, SDL_Renderer *renderer) {
  ...
  SDL_SetRenderTarget(renderer, texture);
  SDL_RenderReadPixels(renderer, NULL, SDL_PIXELFORMAT_RGBA32, pixels,
                       w * sizeof(Uint32));
  SDL_SetRenderTarget(renderer, NULL);
...
void set_pixel(SDL_Surface *surface, Uint8 r, Uint8 g, Uint8 b, Uint8 a,
